 ...
 * cap). `sessions/.files/<uuid>/source/` is already 62 characters before the
 ...
 */
export const SOURCE_NAME_CAP = 72;
 ...
 * source carries — so this only takes out what a path cannot hold.
 *
 ...
 * are one file there and two sources would silently become one.
 */
export function sourceName(raw: string, taken: Set<string>): string {
  const cleaned = (raw.split(/[\\/]/).pop() ?? 'source').replace(/[^\p{L}\p{N}._ -]+/gu, '-');
  const dot = cleaned.lastIndexOf('.');
  const ext = dot > 0 && cleaned.length - dot <= 6 ? cleaned.slice(dot) : '';
  const stem = ext ? cleaned.slice(0, dot) : cleaned;
  const base = windowsSafeName(stem.slice(0, SOURCE_NAME_CAP - ext.length) + ext);

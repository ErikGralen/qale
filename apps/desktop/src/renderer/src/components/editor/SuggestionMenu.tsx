...
export function SuggestionMenu({
  ...
  if (items.length === 0) {
    return <p className="px-2 py-1.5 text-sm text-muted-foreground">{emptyLabel}</p>;
  }

  // Preserve item order while sectioning; group is optional and contiguous.
  const sections: { group: string | undefined; entries: { item: SuggestionMenuItem; index: number }[] }[] = [];

...
function seedSession(dir: string, id: string, sessionType = 'chat'): SessionManager {
  const manager = SessionManager.create(process.cwd(), dir, { id });
  manager.appendCustomEntry('pm.session', { sessionType });
...
test('AgentRuntime lists stored chats with their session type and replays history', async () => {
  ...
  try {
    ...
    manager.appendCustomEntry('pm.session', { sessionType: 'ask' });
    ...
    assert.equal(chats[0]!.sessionType, 'ask');

...
test('isRepo is false for a vault nested inside another repo (regression: vault commits leaked into the source repo)', async () => {
  ...
  const log = await simpleGit(outer)
    .log()
    .catch(() => null);
...
test('session working files are ignored — the memory is versioned, scratch is not', async () => {
  ...
  await git.commitPaths(
    ['sessions/2026-07-28-synthesis-a1b2c3.md', 'sessions/.files/a1b2c3/brief.md'],
    'session: synthesis',
  );
 ...
 */
async function fakeXcodeSelect(
  exitCode: number,
): Promise<{ dir: string; calls: () => Promise<number> }> {
...
test(
  'macOS: no developer tools means no git — history stays empty and the stub is never run',
...
test(
  'the availability probe is cached — a save does not spawn a process per call',
  { skip: process.platform !== 'darwin' },
  async () => {
    const vault = await tmp();
    const git = new GitAdapter(vault);

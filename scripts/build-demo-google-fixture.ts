/**
 * Bake the Rota calendar cast into `demo/google-fixture.json`, the seed the
 ...
 * for `singleEvents=true`, so it never sees an RRULE. Three concrete Steering
...
const SELF = { email: 'demo@rota.example', displayName: 'Demo user' };
 ...
 *  Tech Lead on Scheduling…"). An address with no note keeps its local part. */
...
/** How many instances an RRULE with a COUNT asks for. Weekly is the only
 *  frequency the cast uses; anything else would need a real expander. */
function weeklyCount(recurrence: string[] | undefined): number {
  if (!recurrence?.length) return 1;
...
function eventsFor(m: CastMeeting, names: Map<string, string>): FixtureEvent[] {
  ...
  const count = weeklyCount(m.recurrence);
  const out: FixtureEvent[] = [];
  for (let i = 0; i < count; i += 1) {
    const date = shiftDate(m.date, i * 7);
    const start = {

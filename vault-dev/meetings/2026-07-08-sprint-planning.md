...
([[decisions/2026-07-08-pilot-scheduled-exports]]). Decided against any mobile work this
...
- Exports v1 scope agreed: schedule plus recipient list on an existing dashboard, rendered to
  PDF. Explicitly out: report builder, CSV (fast follow), per-recipient filtering.
...
- Mobile: nothing this quarter. Before Q4 planning, pull mobile-web usage and support-ticket
  counts so we're arguing from data.
- SCIM scoping starts August, after SSO settles. Tom flagged capacity: exports and SCIM in the
  same sprint window will be tight, and exports v1 scope is what gives if anything has to.
